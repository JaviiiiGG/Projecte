...
const getConfig = (getText, path) => {
    ...
    try {
        ...
        if (c) {
            ...
            if (typeof c.lineComment === 'string')
                config.line = c.lineComment;
            else
                warnings.push("lineComment not a string.");
            ...
            if (Array.isArray(c.blockComment) && c.blockComment.length > 1)
                ...
                warnings.push("blockComment not a length-2 array.");
        }
        else
            warnings.push("No comments block found.");
    }
    catch (err) {
        error = err;
    }
...
function default_1(exts, getFileText) {
...
//# sourceMappingURL=CustomLanguage.js.map
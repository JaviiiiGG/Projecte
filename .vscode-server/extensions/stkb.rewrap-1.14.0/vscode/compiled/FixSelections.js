 ...
 *  applied. */
function default_1(oldLines, selections, edit) {
    if (!edit || !edit.lines.length)
        return selections;
    selections = selections.map(s => new vscode.Selection(s.anchor.line, s.anchor.character, s.active.line, s.active.character));
    let runningLineGrowth = 0;
    const { startLine, endLine } = edit, newStartLine = startLine + runningLineGrowth, oldLineCount = endLine - startLine + 1, diff = fd(oldLines.join('\n'), edit.lines.join('\n')), newLineCount = edit.lines.length, rangeLineGrowth = newLineCount - oldLineCount;
